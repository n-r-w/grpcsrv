...
const (
	...
	Greeter_SayManyHellos_FullMethodName = "/api.Greeter/SayManyHellos"
)
...
func (c *greeterClient) SayManyHellos(ctx context.Context, in *HelloRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[HelloResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Greeter_ServiceDesc.Streams[0], Greeter_SayManyHellos_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type Greeter_SayManyHellosClient = grpc.ServerStreamingClient[HelloResponse]
...
func (UnimplementedGreeterServer) SayManyHellos(*HelloRequest, grpc.ServerStreamingServer[HelloResponse]) error {
...
func _Greeter_SayManyHellos_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(GreeterServer).SayManyHellos(m, &grpc.GenericServerStream[HelloRequest, HelloResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Greeter_SayManyHellosServer = grpc.ServerStreamingServer[HelloResponse]
